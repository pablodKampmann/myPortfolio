...
export default function HomeMobile() {
        ...
        const updateClassesTones = () => {
            ...
            switch (colorMain) {
                case 'emerald':
                    ...
                    textLinkColor = tone === 'dark' ? "text-emerald-600" : "text-emerald-600";
                ...
                case 'yellow':
                    ...
                    textLinkColor = tone === 'dark' ? "text-yellow-600" : "text-yellow-600";
    ...
    return (
        <div className={`${classes?.textColorMain} flex flex-col justify-center items-center h-full pb-[6%]`}>
            <div className={`flex justify-start sm:text-sm md:text-base lg:text-lg xl:text-xl 2xl:text-2xl tracking-wide border-b-2 ${classes?.borderColorImage} w-[50%]`}>
                <button onClick={() => setSelected('developer')} className={`${selected === 'developer' ? `${classes?.bgMain}` : `hover:bg-opacity-20 bg-white ${classes?.bgOpacityMain}`} transition duration-100  px-3 py-1 rounded-tl-lg`}>Desarrollador</button>
                <button onClick={() => setSelected('studies')} className={`${selected === 'studies' ? `${classes?.bgMain}` : `hover:bg-opacity-20 bg-white ${classes?.bgOpacityMain}`} transition duration-100  px-3 `}>Estudios</button>
                <button onClick={() => setSelected('skills')} className={`${selected === 'skills' ? `${classes?.bgMain}` : ` hover:bg-opacity-20 bg-white ${classes?.bgOpacityMain}`} transition duration-100  px-3 rounded-tr-lg`}>Habilidades</button>
            </div>
            <div className={`w-[50%] h-[65%] flex flex-col justify-start  p-8 pt-6 relative bg-white ${classes?.bgOpacityMain} rounded-b-2xl rounded-tr-2xl shadow-2xl`}>
                ...
                <div>
                    ...
                    <h1 className={`${classes?.textMainColor} font-semibold md:text-xs lg:text-xs xl:text-sm 2xl:text-base `}>Pablo Kampmann</h1>
                </div>
                {selected === 'developer' && (
                ...
                {selected === 'studies' && (
                    <div className='w-[70%] h-fit space-y-4 overflow-y-auto  md:text-xs lg:text-sm xl:text-base 2xl:text-lg about-me-container  mt-4'>
                        <p className={`${classes?.bgTextInfo} ${classes?.bgOpacityTextInfo}  py-1 px-2  rounded-md`}>{texts?.text_4_part_1} <a onClick={() => window.open('https://uap.edu.ar/', '_blank')} className={`${classes?.textLinkColor} ${classes?.hoverTextLinkColor} hover:cursor-pointer transition duration-150`}>{texts?.text_link_4}</a>{texts?.text_4_part_2}</p>
                        <p className={`${classes?.bgTextInfo} ${classes?.bgOpacityTextInfo}  py-1 px-2  rounded-md`}>{texts?.text_5}</p>
                        <h2 className={`flex md:text-xs lg:text-sm xl:text-base 2xl:text-xl items-center underline ${classes?.decorationColor} ml-1`}>Titulos <IoSchool className='ml-2' /></h2>
                        <p className={`${classes?.bgTextInfo} ${classes?.bgOpacityTextInfo}  py-1 px-2 w-fit  rounded-md`}>{texts?.degree_1} <span className={`${classes?.textMainColor}`}>90%</span></p>
                        <p className={`${classes?.bgTextInfo} ${classes?.bgOpacityTextInfo}  py-1 px-2 w-fit  rounded-md`}>{texts?.degree_2} <span className={`${classes?.textMainColor}`}>70%</span></p>

                    </div>
                )}
                {selected === 'skills' && (
                    <div className='w-[70%] h-fit space-y-4 overflow-y-auto md:text-xs lg:text-sm xl:text-base 2xl:text-lg about-me-container mt-4'>
                        <div className={`${classes?.bgTextInfo} ${classes?.bgOpacityTextInfo} py-1 px-2 rounded-lg flex justify-between items-center`}>
                ...
                <div className='absolute -top-10 -right-10'>
                    <Image placeholder='blur' blurDataURL={imageBlur} className={`rounded-full border-4  ${classes?.borderColorImage}  shadow-2xl object-cover w-[200px] h-[200px]`} quality={10} width={959} height={1280} priority={true} src='/me-image.jpg' alt="me-image" />
                    {openSocial && (
                        <div className=''>
