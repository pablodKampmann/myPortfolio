...
export default function DesktopVersion({
    ...
    const updateClassesTones = () => {
      ...
      switch (colorMain) {
        case "emerald":
          ...
          bgPageSelected =
            tone === "dark" ? "bg-emerald-600" : "bg-emerald-600";
          textColorNumber =
  ...
  return (
    <div
      className={`w-full flex z-50 relative justify-between items-center ${classes?.bgMain} border-b-2 ${classes?.borderColor} transition select-none duration-200 ${classes?.bgMainOpacity} h-16 `}
    >
      <div className="ml-4 flex justify-center items-center">
        {showCancelButton ? (
          <div
            className={`cursor-pointer  ${classes?.hoverBgColorIcon} ${classes?.hoverBgOpacityIcon} transition animate-[spin_0.5s_ease-out] duration-300 p-1 rounded-full`}
          >
            <MdOutlineCancel
              onClick={() => {
                setShowOptions(false);
          ...
          <div
            className={`cursor-pointer ${isVisible ? `${classes?.bgColorButtons} bg-opacity-10` : "bg-transparent"} ${classes?.hoverBgColorIcon} ${classes?.hoverBgOpacityIcon} transition duration-300  p-1 rounded-full`}
          >
            <SiVorondesign
        ...
        >
          ...
          >
            {texts?.select_the}{" "}
            <span className={`${classes?.textIcon} font-semibold text-sm`}>
              {texts?.color}
            </span>{" "}
            {texts?.tatata}
          </h1>
          <div className="flex flex-col justify-center space-y-2 items-center">
            <div className="flex justify-center items-center space-x-4">
              <button
                onClick={() => {
                  handleColorChange("emerald");
                  setShowOptions(false);
                  setShowCancelButton(false);
                }}
                className={`${colorMain === "emerald" ? `w-8 h-8 ${classes?.borderOpacityColourSelected} ${classes?.borderColorColourSelected}` : "w-8 h-8 hover:scale-125 border-transparent transition duration-150"} bg-emerald-600 border-2 transition duration-200 shadow-2xl rounded-full`}
              ></button>
              <button
                onClick={() => {
                  handleColorChange("rose");
                  setShowOptions(false);
                  setShowCancelButton(false);
                }}
                className={`${colorMain === "rose" ? `w-8 h-8 ${classes?.borderOpacityColourSelected} ${classes?.borderColorColourSelected}` : "w-8 h-8 hover:scale-125 border-transparent transition duration-150"} bg-rose-600 border-2 transition duration-200 shadow-2xl rounded-full`}
              ></button>
            </div>
            <div className="flex justify-center items-center space-x-4">
              <button
                ...
                className={`${colorMain === "blue" ? `w-8 h-8 ${classes?.borderOpacityColourSelected} ${classes?.borderColorColourSelected}` : "w-8 h-8 border-transparent hover:scale-125 transition duration-150"} bg-blue-600 border-2 transition duration-200 shadow-2xl rounded-full`}
              ></button>
              <button
                onClick={() => {
                ...
                className={`${colorMain === "yellow" ? `w-8 h-8 ${classes?.borderOpacityColourSelected} ${classes?.borderColorColourSelected}` : "w-8 h-8 border-transparent hover:scale-125 transition duration-150"} bg-yellow-600 border-2 transition duration-200 shadow-2xl rounded-full`}
              ></button>
            </div>
          </div>
        </div>
      ...
      <div
        className={`${classes?.textMain} flex space-x-4 md:text-xs lg:text-sm xl:text-base 2xl:text-lg  justify-end items-center mr-4`}
      >
        <Link href={"/"} prefetch={true}>
          <div
            className={`${pathname === "/" ? `${classes?.bgPageSelected}  ${classes?.bgOpacityButtonPressed} ` : `${classes?.bgOpacityHoverButtons} ${classes?.bgColorButtons} ${classes?.bgOpacityButtons} `} cursor-pointer  focus:outline-none  py-2 px-3 rounded-lg transition duration-150  w-full whitespace-nowrap`}
          >
            <span className={`${classes?.textColorNumber}`}>01. </span>
            {texts?.about}
          </div>
        </Link>
        <Link href={"/work"} prefetch={true}>
          <div
            className={`${pathname === "/work" ? `${classes?.bgPageSelected}  ${classes?.bgOpacityButtonPressed}` : `${classes?.bgOpacityHoverButtons} ${classes?.bgColorButtons} ${classes?.bgOpacityButtons} `} cursor-pointer focus:outline-none py-2 px-3 rounded-lg transition duration-150  w-full whitespace-nowrap`}
          >
            <span className={`${classes?.textColorNumber}`}>02. </span>
            {texts?.work}
          </div>
        </Link>
        <Link href={"/tech"} prefetch={true}>
          <div
            className={`${pathname === "/tech" ? `${classes?.bgPageSelected}  ${classes?.bgOpacityButtonPressed}` : `${classes?.bgOpacityHoverButtons} ${classes?.bgColorButtons} ${classes?.bgOpacityButtons}`} cursor-pointer  focus:outline-none py-2 px-3 rounded-lg transition duration-150  w-full whitespace-nowrap`}
          >
            <span className={`${classes?.textColorNumber}`}>03. </span>
            {texts?.technologies}
          </div>
        </Link>
        <div
          className={`bg-transparent uppercase font-semibold whitespace-nowrap white tracking-widest border-2  ${classes?.textColorNumber} hover:border-white ${classes?.hoverButtton} hover:bg-opacity-70 hover:text-white transition duration-150 cursor-pointer flex justify-center items-center ${classes?.borderColor} rounded-md py-1 px-3 `}
        >
          {texts?.repo}
