...
export default function DesktopVersion({ tone, language, colorMain, handleColorChange }: NavBarProps) {
    ...
    return (
        <div className={`w-full flex z-50 relative justify-between items-center ${classes?.bgMain} border-b-2 ${classes?.borderColor} transition select-none duration-200 ${classes?.bgMainOpacity} h-16 `} >
            ...
            <div className={`${classes?.textMain} flex space-x-4 md:text-xs lg:text-sm xl:text-base 2xl:text-lg  justify-end items-center mr-4`}>
                <Link href={'/'} prefetch={true}>
                    <div className={`${pathname === '/' ? `${classes?.bgPageSelected}  ${classes?.bgOpacityButtonPressed} ` : `${classes?.bgOpacityHoverButtons} ${classes?.bgColorButtons} ${classes?.bgOpacityButtons} `} cursor-pointer  focus:outline-none  py-2 px-3 rounded-lg transition duration-150  w-full whitespace-nowrap`}><span className={`${classes?.textColorNumber}`}>01. </span>{texts?.about}</div>
                </Link>
                <Link href={'/work'} prefetch={true}>
                    <div className={`${pathname === '/work' ? `${classes?.bgPageSelected}  ${classes?.bgOpacityButtonPressed}` : `${classes?.bgOpacityHoverButtons} ${classes?.bgColorButtons} ${classes?.bgOpacityButtons} `} cursor-pointer focus:outline-none py-2 px-3 rounded-lg transition duration-150  w-full whitespace-nowrap`}><span className={`${classes?.textColorNumber}`}>02. </span>{texts?.work}</div>
                </Link>
                <Link href={'/tech'} prefetch={true}>
                    <div className={`${pathname === '/tech' ? `${classes?.bgPageSelected}  ${classes?.bgOpacityButtonPressed}` : `${classes?.bgOpacityHoverButtons} ${classes?.bgColorButtons} ${classes?.bgOpacityButtons}`} cursor-pointer  focus:outline-none py-2 px-3 rounded-lg transition duration-150  w-full whitespace-nowrap`}><span className={`${classes?.textColorNumber}`}>03. </span>{texts?.technologies}</div>
                </Link>
                <div className={`bg-transparent uppercase font-semibold whitespace-nowrap white tracking-widest border-2  ${classes?.textColorNumber} hover:border-white ${classes?.hoverButtton} hover:bg-opacity-70 hover:text-white transition duration-150 cursor-pointer flex justify-center items-center ${classes?.borderColor} rounded-md py-1 px-3 `}>{texts?.repo}<LuGithub className="ml-2" size={34} /></div>
